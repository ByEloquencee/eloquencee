...
interface AntonymQuizViewProps {
  words: PolishWord[];
  allWords: PolishWord[];
  onExit: () => void;
...
export function AntonymQuizView({ words, allWords, onExit, onComplete }: AntonymQuizViewProps) {
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      const focus = shuffle(words).slice(0, 30).map((w) => ({
        word: w.word,
        definition: w.definition,
        part_of_speech: w.partOfSpeech,
      }));
      // Pool = all app words (capped at 250 server-side)
      const pool = shuffle(allWords).slice(0, 250).map((w) => ({
        word: w.word,
        definition: w.definition,
        part_of_speech: w.partOfSpeech,
      }));

      const { data, error: fnError } = await supabase.functions.invoke("generate-antonym-quiz", {
        body: { words: focus, pool },
      });
      ...
      console.error(e);
      setError(true);
      toast.error("Nie udało się wygenerować quizu antonimów");
    } finally {
      setLoading(false);
    }
  }, [words, allWords]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 gap-3">
        ...
        <p className="text-sm text-muted-foreground">Generowanie quizu antonimów...</p>
      </div>
    );
  }
  ...
  if (error || questions.length === 0) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 gap-4">
        <p className="text-sm text-muted-foreground text-center">Nie udało się załadować quizu.</p>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={current} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="w-full relative z-10 space-y-6">
            ...
            <div className="p-5 rounded-2xl bg-secondary/50 border border-border">
              <p className="text-xs text-muted-foreground mb-2 font-medium">Znajdź przeciwieństwo słowa o definicji:</p>
              <p className="text-base font-medium text-foreground leading-relaxed">{question.question_definition}</p>
            </div>
            ...
            {/* Reveal pytanego słowa + wyjaśnienie */}
            <AnimatePresence>
              {selected !== null && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="overflow-hidden space-y-2">
                  <div className="p-3 rounded-xl bg-primary/10 text-foreground text-sm">
                    <span className="text-xs text-muted-foreground">Pytane słowo: </span>
                    <span className="font-semibold">{question.question_word}</span>
                  </div>
                  <div className={`flex items-start gap-2 p-3 rounded-xl text-sm ${
                    ...
                    {selected === question.correct ? <Check size={16} className="flex-shrink-0 mt-0.5" /> : <X size={16} className="flex-shrink-0 mt-0.5" />}
            ...
            {selected !== null && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-xs text-muted-foreground cursor-pointer" onClick={handleAdvance}>
